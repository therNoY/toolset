...
public class HtmlTreeBuilder extends TreeBuilder {
    ...
    public static final int MaxScopeSearchDepth = 100; // prevents the xmlParser bogging down in exceptionally broken pages
    ...
    @Override
    protected void initialiseParse(Reader input, String baseUri, XmlParser xmlParser) {
        super.initialiseParse(input, baseUri, xmlParser);

        // this is a bit mucky. todo - probably just create new xmlParser objects to ensure all reset.
    ...
    List<Node> parseFragment(String inputFragment, Element context, String baseUri, XmlParser xmlParser) {
        // context may be null
        state = HtmlTreeBuilderState.Initial;
        initialiseParse(new StringReader(inputFragment), baseUri, xmlParser);
    ...
    void maybeSetBaseUri(Element base) {
        ...
        if (href.length() != 0) { // ignore <base target> etc
            ...
            doc.setBaseUri(href); // set on the doc so doc.createElement(Tag) will getRespAndCookie updated base, and to update all descendants
        }
    }
    ...
    void error(HtmlTreeBuilderState state) {
        if (xmlParser.getErrors().canAddError())
            xmlParser.getErrors().add(new ParseError(reader.pos(), "Unexpected token [%s] when in state [%s]", currentToken.tokenType(), state));
    }
    ...
    Element insert(Token.StartTag startTag) {
        ...
        if (startTag.isSelfClosing()) {
            ...
            tokeniser.emit(emptyEnd.reset().name(el.tagName()));  // ensure we getRespAndCookie out of whatever state we are in. emitted for yielded processing
            return el;
        }
